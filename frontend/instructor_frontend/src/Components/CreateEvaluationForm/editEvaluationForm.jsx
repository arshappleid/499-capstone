	...
	const fetchForm = async () => {
		try {
			...
			const response = await axios.post(
				BACKEND_URL + 'instructor/instructorgetform',
				requestData,
				{ headers: headers }
			);
			const allForms = {
...
export function EditEvaluationForm() {
	useEffect(() => {
		setModalTitle2('Attention!');
		setModalContent2(
			'Please submit the form before you leave, otherwise your changes will be lost.'
		);
	...
	// const [value, setValue] = React.useState(dayjs().endOf('day'));
	const formData = FetchFormData();
	const formdataDeadline = dayjs.utc(formData.deadline);
	const [value, setValue] = React.useState(formdataDeadline);
	useEffect(() => {
		setValue(formdataDeadline);
	}, [formData]);
					...
					questions: Yup.array()
						.of(
							Yup.object().shape({
								...
								// weight: Yup.number()
								//   .typeError("Question Weight must be a number")
								//   .min(1, "Question Weight cannot be 0 or negative"),
								options: Yup.array()
									.of(
										Yup.string().required(
					...
					<Formik
						...
						onSubmit={async (values) => {
							try {
								...
								if (status === 'success') {
									setModalTitle('Form Edited Successfully!');
									setModalContent(
									...
									setModalTitle(
										'Error! Evaluation From could not be edited!'
									);
								...
								<Form>
									<Space className=" flex flex-col justify-start items-start w-full mt-10">
										...
										<ol className="text-lg font-futura ml-14 -mt-4 list-disc list-inside">
											...
											<li>
												Questions that have option
												point(s) should have a numeric
												value (could be a decimal).
											</li>
											<li>
									...
									<Space className="flex flex-row items-start justify-items-start w-full">
										...
										<LocalizationProvider
											...
											<div className="text-red font-futura ml-2">
												<DateTimePicker
													className="ml-2 mb-10 mt-10 font-futura text-lg p-0 bg-white border-2 border-slate-200 rounded-md"
													id="deadline"
													name="deadline"
													...
													(section, sectionIndex) => (
														<div
															...
															<Space className="flex flex-row justify-between">
																<h2 className="font-futura text-xl font-bold pt-4 mb-10 ml-6 h-10">
																	Section{' '}
																	{String.fromCharCode(
																		65 +
																			sectionIndex
																				...
																				<div
																					...
																					{question.type ===
																						...
																						<div>
																							...
																							<Field
																								...
																								className="font-futura text-lg border-2  w-3/5 ml-2 pl-1 rounded-md"
																					...
																					{question.type ===
																						...
																						<div>
																							...
																							<Field
																								...
																								id={`sections.${sectionIndex}.questions.${questionIndex}.question`}
																								name={`sections.${sectionIndex}.questions.${questionIndex}.question`}
																								className=" font-futura border-2 text-lg w-3/5 ml-2 pl-1 h-8 rounded-md"
																								placeholder="Enter Question Text"
																							/>
																							<ErrorMessage
																								name={`sections.${sectionIndex}.questions.${questionIndex}.question`}
																										...
																										{question.options.map(
																											(
																												...
																												<Field
																													...
																													id={`sections.${sectionIndex}.questions.${questionIndex}.optionPoints.${optionIndex}`}
																													name={`sections.${sectionIndex}.questions.${questionIndex}.optionPoints.${optionIndex}`}
																													className="font-futura text-lg w-1/12 ml-2"
																												/>
																												<ErrorMessage
																													name={`sections.${sectionIndex}.questions.${questionIndex}.optionPoints.${optionIndex}`}
																													component="div"
																					...
																					{question.type ===
																						...
																						<div>
																							...
																							<Field
																								...
																								id={`sections.${sectionIndex}.questions.${questionIndex}.question`}
																								name={`sections.${sectionIndex}.questions.${questionIndex}.question`}
																								className=" font-futura border-2 text-lg w-4/5 ml-2 pl-1 h-8 rounded-md"
																								placeholder="Enter Question Text"
																							/>
																							<ErrorMessage
																								name={`sections.${sectionIndex}.questions.${questionIndex}.question`}
																					...
																					{question.type ===
																						...
																						<div>
																							...
																							/>
																							{/* <ErrorMessage
                                        name={`sections.${sectionIndex}.questions.${questionIndex}.weight`}
                                        component="div"
                                        className="inline font-futura border-blacke-500 text-lg ml-5 mt-1 text-red-500"
                                      /> */}
																							<br />
																							<Space className="flex flex-row w-full justify-start mt-10">
																								...
																								<div>
																									<Button
																										...
																										className="text-base h-auto font-futura font-bold rounded-lg w-auto ml-14 drop-shadow-2xl bg-blue-500 hover:bg-blue-400 text-white">
																										<span className="text-white hover:text-white">
																											Likert
																											Scale
																										</span>
																									...
																									<Button
																										...
																										className="text-base h-auto font-futura font-bold rounded-lg w-auto ml-4 drop-shadow-2xl bg-blue-500 hover:bg-blue-400 text-white">
																										<span className="text-white hover:text-white">
																											0-4
																											Scale
																										</span>
									...
									<Tooltip
										title={
											<span className="text-base font-futura">
												{isValid
													? 'Submit Form'
													: 'Complete the form in order to submit'}
											</span>
										}>
										<div className="flex justify-center">
											<Button
												...
												disabled={!isValid}>
												<span className="text-white text-base hover:text-white font-futura">
													Submit
												</span>
											</Button>
										</div>
