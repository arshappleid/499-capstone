...
describe('addStudentToCoursePreview', async () => {
	const instructor_id = 3235;
	const student_id1 = 3235;
	const student_id2 = 6235;
	const student_id3 = 9235;
	const student_id4 = 12235;
	let course_id;
	let group_id;
	let other_group_id;
	...
	beforeEach(async () => {
		try {
			...
			await Instructor.create({
				...
				MIDDLE_NAME: 'MIDDLE_NAME',
				LAST_NAME: 'LAST_NAME',
				EMAIL: 'EMAIL3235@TEST.COM',
				MD5_HASHED_PASSWORD: 'MD5_PASSWORD',
			});
			const courseCreated = await Course.create({
				INSTRUCTOR_ID: instructor_id,
				COURSE_NAME: 'TEST_COURSE_NAME3235',
				COURSE_CODE: 'COURSE_CODE3235',
				COURSE_SEMESTER: 'COURSE_SEMESTER3235',
				COURSE_YEAR: 'COURSE_YEAR3235',
				COURSE_TERM: 'COURSE_TERM3235',
				COURSE_VISIBILITY: 0,
			});
	...
	test('Undefined Variable', async () => {
		const result = await addStudentToGroupPreviewService();
