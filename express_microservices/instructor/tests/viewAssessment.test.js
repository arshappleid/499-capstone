...
describe('instructorViewAssignmentAssessment', () => {
	const ticket_num1 = 400;
	const ticket_num2 = 4001;
	const ticket_num3 = 4002;
	const ticket_num4 = 4003;
	const ticket_num5 = 4004;
    ...
    test('instructor not exist', async () => {
        const result = await instructorViewAssignmentAssessmentService(0, course_id, assignment_id1, student_id2, student_id1);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Instructor does not exist');
    });
    test('course not exist', async () => {
        const result = await instructorViewAssignmentAssessmentService(instructor_id, 0, assignment_id1, student_id2, student_id1);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Course does not exist');
    });
    test('assignment not exist', async () => {
        const result = await instructorViewAssignmentAssessmentService(instructor_id, course_id, 0, student_id2, student_id1);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Assignment does not exist');
    });
    test('Evaluator does not exist', async () => {
        const result = await instructorViewAssignmentAssessmentService(instructor_id, course_id, assignment_id1, 0, student_id1);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Evaluator does not exist');
    });
    test('Evaluatee does not exist', async () => {
        const result = await instructorViewAssignmentAssessmentService(instructor_id, course_id, assignment_id1, student_id2, 0);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Evaluatee does not exist');
    });
    test('Successful', async () => {
        const result = await instructorViewAssignmentAssessmentService(instructor_id, course_id, assignment_id1, student_id2, student_id1);
        expect(result.status).toBe('success');
        expect(result.message).toBe('Assignment and Rubric retrieved');
    });
});
