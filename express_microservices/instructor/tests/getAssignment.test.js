...
describe('instructorGetAssignment', () => {
    const ticket_num1 = 459;
    const ticket_num2 = 4591;
    const ticket_num3 = 4592;
    const ticket_num4 = 4593;
    const ticket_num5 = 4594;
    ...
    test('Undefined values passed to Service Function' , async () => {
        const result = await instructorGetAssignmentService();
    ...
    test('instructor not exist', async () => {
        const result = await instructorGetAssignmentService(0, course_id, assignment_id1);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Instructor does not exist');
    });
    test('course not exist', async () => {
        const result = await instructorGetAssignmentService(instructor_id, 0, assignment_id1);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Course does not exist');
    });
    test('assignment not exist', async () => {
        const result = await instructorGetAssignmentService(instructor_id, course_id, 0);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Assignment does not exist');
    });
    test('Get successfully assignment1', async () => {
        const result = await instructorGetAssignmentService(instructor_id, course_id, assignment_id1);
        expect(result.status).toBe('success');
        expect(result.message).toBe("Assignment retrieved successfully");
        expect(result.assignment.ASSIGNMENT_ID).toBe(assignment_id1);
    });
    test('Get successfully assignment2', async () => {
        const result = await instructorGetAssignmentService(instructor_id, course_id, assignment_id2);
        expect(result.status).toBe('success');
        expect(result.message).toBe("Assignment retrieved successfully");
        expect(result.assignment.ASSIGNMENT_ID).toBe(assignment_id2);
    });
});