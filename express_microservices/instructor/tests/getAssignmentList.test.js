...
describe('getAssignmentList', () => {
	...
	test('Get with undefined values', async () => {
		const assignments = await getAssignmentListService(
			undefined,
			undefined
		);
	...
	test('Course does not exist', async () => {
		const assignments = await getAssignmentListService(instructor_id, -1);
		expect(assignments.status).toBe('error');
		expect(assignments.message).toBe('Course does not exist');
	});
	test('Instructor does not exist', async () => {
		const assignments = await getAssignmentListService(-1, course_id);
		expect(assignments.status).toBe('error');
	...
	test('Get successfully', async () => {
		...
		expect(assignments.data[0].ASSIGNMENT_NAME).toBe(
			'TEST_ASSIGNMENT_NAME'
		);
		expect(assignments.data[0].COURSE_ID).toBe(course_id);
	});
});
