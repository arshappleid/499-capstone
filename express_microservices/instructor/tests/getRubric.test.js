...
describe('instructorGetRubricLink', () => {
    const ticket_num1 = 416;
    const ticket_num2 = 4161;
    const ticket_num3 = 4162;
    const ticket_num4 = 4163;
    const ticket_num5 = 4164;
    ...
    test('instructor not exist', async () => {
        const result = await instructorGetRubricService(0, course_id, assignment_id1);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Instructor does not exist');
    });
    test('course not exist', async () => {
        const result = await instructorGetRubricService(instructor_id, 0, assignment_id1);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Course does not exist');
    });
    test('assignment not exist', async () => {
        const result = await instructorGetRubricService(instructor_id, course_id, 0);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Assignment does not exist');
    });
    test('Get Success', async () => {
        const result = await instructorGetRubricService(instructor_id, course_id, assignment_id1);
