...
describe('instructorGetSubmissionLink', () => {
    const ticket_num1 = 458;
    const ticket_num2 = 4581;
    const ticket_num3 = 4582;
    const ticket_num4 = 4583;
    const ticket_num5 = 4584;
    ...
    test('instructor not exist', async () => {
        const result = await instructorGetSubmissionLinkService(0, course_id, assignment_id1, student_id1);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Instructor does not exist');
    });
    test('course not exist', async () => {
        const result = await instructorGetSubmissionLinkService(instructor_id, 0, assignment_id1, student_id1);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Course does not exist');
    });
    test('assignment not exist', async () => {
        const result = await instructorGetSubmissionLinkService(instructor_id, course_id, 0, student_id1);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Assignment does not exist');
    });
    test('student not exist', async () => {
        const result = await instructorGetSubmissionLinkService(instructor_id, course_id, assignment_id1, 0);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Student does not exist');
    });
    test('get URL successfully', async () => {
        const result = await instructorGetSubmissionLinkService(instructor_id, course_id, assignment_id1, student_id1);
        expect(result.status).toBe('success');
        expect(result.message).toBe('TEST_SUBMISSION_LINK458');
    });
});