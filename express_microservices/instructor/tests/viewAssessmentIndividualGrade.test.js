...
describe('instructorGetAssessmentIndividualGrade', () => {
	const ticket_num1 = 485;
	const ticket_num2 = 4851;
	const ticket_num3 = 4852;
	const ticket_num4 = 4853;
	const ticket_num5 = 4854;
	...
	test('Undefined values passed to Service Function', async () => {
		const reslut = await instructorGetAssessmentIndividualGradeService(
			undefined,
			undefined,
			undefined,
			undefined,
	...
	test('instructor not exist', async () => {
		const result = await instructorGetAssessmentIndividualGradeService(
			0,
			course_id,
			assignment_id1,
			student_id2,
    ...
    test('No assessment yet', async () => {
        ...
        expect(result.status).toBe('error');
        expect(result.message).toBe('Evaluatee is not in any assessment group');
    });
    test('Get successfully', async () => {
        ...
        expect(result.total_grade_in_percentage).toEqual(100);
    });
});
