...
describe('instructorSetAssignmentSharefeedback', () => {
	const instructor_id = 456;
	let course_id;
	let assignment_id;
	...
	beforeEach(async () => {
		await Instructor.create({
			...
			LAST_NAME: 'LAST_NAME',
			EMAIL: 'INSTRUCTOR456@TEST.COM',
			MD5_HASHED_PASSWORD: 'MD5_PASSWORD',
		});
		const courseCreated = await Course.create({
			INSTRUCTOR_ID: instructor_id,
			COURSE_NAME: 'TEST_COURSE_NAME-456',
			COURSE_CODE: 'COURSE_CODE-456',
			COURSE_SEMESTER: 'COURSE_SEMESTER-456',
			COURSE_YEAR: 'COURSE_YEAR-456',
			COURSE_TERM: 'COURSE_TERM-456',
			COURSE_VISIBILITY: 0,
		});
		course_id = courseCreated.get('COURSE_ID');
		const assignmentCreated = await CourseAssignment.create({
			COURSE_ID: course_id,
			ASSIGNMENT_NAME: 'TEST_ASSIGNMENT_NAME456',
