...
describe('instructorSetFormSharefeedback', async () => {
	const instructor_id1 = 232;
	const instructor_id2 = 2321;
	let course_id1;
	let course_id2;
	let form_id;
	...
	beforeEach(async () => {
		try {
			...
			await Instructor.create({
				...
				MIDDLE_NAME: 'MIDDLE_NAME',
				LAST_NAME: 'LAST_NAME',
				EMAIL: 'EMAIL2322@TEST.COM',
				MD5_HASHED_PASSWORD: 'MD5_PASSWORD',
			});
			const courseCreated1 = await Course.create({
				INSTRUCTOR_ID: instructor_id1,
				COURSE_NAME: 'TEST_COURSE_NAME232',
				COURSE_CODE: 'COURSE_CODE232',
				COURSE_SEMESTER: 'COURSE_SEMESTER232',
				COURSE_YEAR: 'COURSE_YEAR232',
				COURSE_TERM: 'COURSE_TERM232',
				COURSE_VISIBILITY: 0,
			});
	...
	test('Undefined Variable', async () => {
		const result = await instructorSetFormShareFeedbackService();
	...
	test('Invalid Variable', async () => {
		const result1 = await instructorSetFormShareFeedbackService(
			instructor_id1,
			course_id1,
			2
		);
		...
		const result2 = await instructorSetFormShareFeedbackService(
			instructor_id1,
			course_id1,
			-1
		);
		...
		const result3 = await instructorSetFormShareFeedbackService(
			instructor_id1,
			course_id1,
			'1'
		);
	...
	test('Instructor not found', async () => {
		const result = await instructorSetFormShareFeedbackService(0, form_id, 1);
	...
	test('Successfully Set to true', async () => {
		...
		expect(result.data.form_sharefeedback).toBe(1);

		const form_sharefeedback = await EvaluationForm.findOne({
			where: { FORM_ID: form_id },
		});
		expect(form_sharefeedback.SHARE_FEEDBACK).toBe(true);
	});
	test('Successfully Set to false', async () => {
		...
		expect(result.status).toBe('success');
		expect(result.data.form_sharefeedback).toBe(0);

		const form_sharefeedback = await EvaluationForm.findOne({
			where: { FORM_ID: form_id },
		});
		expect(form_sharefeedback.SHARE_FEEDBACK).toBe(false);
	});
});
