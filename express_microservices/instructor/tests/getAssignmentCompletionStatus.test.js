...
const instructorGetAssignmentCompletionStatusService = require('../services/instructorGetAssignmentCompletionStatusService');

describe('instructorGetAssignmentCompletionStatus', () => {
    const ticket_num1 = 462;
    const ticket_num2 = 4621;
    const ticket_num3 = 4622;
    const ticket_num4 = 4623;
    const ticket_num5 = 4624;
    ...
    test('Undefined values passed to Service Function' , async () => {
        const result = await instructorGetAssignmentCompletionStatusService();
    ...
    test('instructor not exist', async () => {
        const result = await instructorGetAssignmentCompletionStatusService(0, course_id, assignment_id1);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Instructor does not exist');
    });
    test('course not exist', async () => {
        const result = await instructorGetAssignmentCompletionStatusService(instructor_id, 0, assignment_id1);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Course does not exist');
    });
    test('assignment not exist', async () => {
        const result = await instructorGetAssignmentCompletionStatusService(instructor_id, course_id, 0);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Assignment does not exist');
    });
    test('get successfully assignment1', async () => {
        const result = await instructorGetAssignmentCompletionStatusService(instructor_id, course_id, assignment_id1);
        expect(result.status).toBe('success');
        expect(result.studentCompletionStatus.length).toBe(1);
        expect(result.studentCompletionStatus[0].student_id).toBe(student_id1);
        expect(result.studentCompletionStatus[0].status).toBe('complete');
        expect(result.studentCompletionStatus[0].submission_link).toBe('TEST_SUBMISSION_LINK' + ticket_num1);
    });
    test('get successfully assignment2', async () => {
        const result = await instructorGetAssignmentCompletionStatusService(instructor_id, course_id, assignment_id2);
        expect(result.status).toBe('success');
        expect(result.studentCompletionStatus.length).toBe(1);
        expect(result.studentCompletionStatus[0].student_id).toBe(student_id1);
        expect(result.studentCompletionStatus[0].status).toBe('incomplete');
    });
});