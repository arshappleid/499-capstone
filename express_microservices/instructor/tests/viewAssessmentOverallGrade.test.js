...
describe('instructorGetAssessmentOverallGradeService', () => {
	const ticket_num1 = 487;
	const ticket_num2 = 4871;
	const ticket_num3 = 4872;
	const ticket_num4 = 4873;
	const ticket_num5 = 4874;
    ...
    test('Undefined values passed to Service Function', async () => {
		const reslut = await instructorGetAssessmentOverallGradeService(
			undefined,
			undefined,
			undefined,
			undefined
    ...
    test('No assessment yet', async () => {
        ...
        expect(result.status).toBe('error');
        expect(result.message).toBe('Assessment Group does not exist');
    });
    test('Get successfully', async () => {
        ...
        expect(result.overall_total_grade_in_percentage).toEqual(100);
    }); 
});