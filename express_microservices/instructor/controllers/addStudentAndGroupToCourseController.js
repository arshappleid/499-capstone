		...
		for (let i = 0; i < groups.length; i++) {
			...
			if (
				...
				if (groupExist.status) {
					...
					const created_group = await instructorCreateGroupService(
						course_id,
						group_name
					);
					if (created_group.status === 'error') {
						data.push({
							group_name: group_name,
							message:
								'group create failed:' + created_group.message,
						});
					} else {
						let student_status = [];
						...
						for (let j = 0; j < student_ids.length; j++) {
							...
							const studentExist =
								await checkIfStudentExistService(student_id);
							...
							if (studentAddStatus.status === 'error') {
								...
								studentAddStatus.status === 'duplicate'
							) {
								student_status.push({
									student_id: student_id,
						...
						const autoRemoveEmptyGroup =
							await removeEmptyGroupService(
								instructor_id,
								course_id,
								created_group.data.group_id
							);

						if (autoRemoveEmptyGroup.status === 'error') {
							data.push({
								group_name: group_name,
							...
							data.push({
								group_name: group_name,
								status: 'No student added. Group deleted automatically.',
								student_status: student_status,
							});
						}
					}
