...
async function addStudentToGroupService(course_id, group_id, student_id) {
	try {
		if (
			course_id === undefined ||
			group_id === undefined ||
		...
		const newStudentGroup = await GroupMembersTable.create({
			STUDENT_ID: student_id,
			GROUP_ID: group_id,
		});
		return {
			status: 'success',
			data: { student_id: newStudentGroup.STUDENT_ID },
		};
	} catch (e) {
		console.log('Error in addStudentToCourseService: ', e);
		return {
			status: 'error',
			message: 'Error occured while retrieving results in addStudentToCourseService.js: ' + e.message,
		};
	}
}

module.exports = addStudentToGroupService;
