...
describe('instructorGetRubricLink', () => {
    const ticket_num1 = 417;
    const ticket_num2 = 4171;
    const ticket_num3 = 4172;
    const ticket_num4 = 4173;
    const ticket_num5 = 4174;
    ...
    test('Student not exist', async () => {
        const result = await getRubricService(0, course_id, assignment_id1);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Student does not exist');
    });
    test('course not exist', async () => {
        const result = await getRubricService(student_id1, 0, assignment_id1);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Course does not exist');
    });
    test('assignment not exist', async () => {
        const result = await getRubricService(student_id1, course_id, 0);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Assignment does not exist');
    });
    test('Get Success', async () => {
        const result = await getRubricService(student_id1, course_id, assignment_id1);
