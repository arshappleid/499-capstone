...
describe('Testing getStudentToAssessService', async () => {
	const ticket_num1 = 435;
	const ticket_num2 = 4351;
	const ticket_num3 = 4352;
	const ticket_num4 = 4353;
	const ticket_num5 = 4354;
    ...
    test('Undefined values passed to Service Function', async () => {
		const reslut = await getStudentToAssessService(
			undefined,
			undefined
		);
    ...
    test('Get Successfully', async () => {
        ...
        expect(reslut.studentToAssess[0]).toBe(student_id1);
    });
    test('Get Successfully', async () => {
        const reslut = await getStudentToAssessService(
            student_id1,
        ...
        expect(reslut.status).toBe('success');
        expect(reslut.studentToAssess.length).toBe(0);
    });
});