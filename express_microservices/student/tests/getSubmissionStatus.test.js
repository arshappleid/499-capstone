...
describe('studentGetSubmissionStatus', () => {
    const ticket_num1 = 493;
    const ticket_num2 = 4931;
    const ticket_num3 = 4932;
    const ticket_num4 = 4933;
    const ticket_num5 = 4934;
    ...
    test('student not exist', async () => {
        const result = await getSubmissionStatusService(0, course_id);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Student does not exist');
    });
    test('course not exist', async () => {
        const result = await getSubmissionStatusService(student_id1, 0);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Course does not exist');
    });
    test('get submission status successfully', async () => {
        const result = await getSubmissionStatusService(student_id1, course_id);
        expect(result.status).toBe('success');
        expect(result.assignments_status.length).toBe(2);
    });
});
