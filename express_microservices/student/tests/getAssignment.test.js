...
describe('studentGetAssignment', () => {
    const ticket_num1 = 498;
    const ticket_num2 = 4981;
    const ticket_num3 = 4982;
    const ticket_num4 = 4983;
    const ticket_num5 = 4984;
    ...
    test('student not exist', async () => {
        const result = await getAssignmentService(0, course_id, assignment_id1);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Student does not exist');
    });
    test('course not exist', async () => {
        const result = await getAssignmentService(instructor_id, 0, assignment_id1);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Course does not exist');
    });
    test('assignment not exist', async () => {
        const result = await getAssignmentService(instructor_id, course_id, 0);
        expect(result.status).toBe('error');
        expect(result.message).toBe('Assignment does not exist');
    });
    test('Get successfully assignment1', async () => {
        const result = await getAssignmentService(instructor_id, course_id, assignment_id1);
    ...
    test('Get successfully assignment2', async () => {
        const result = await getAssignmentService(instructor_id, course_id, assignment_id2);
        expect(result.status).toBe('success');
        expect(result.message).toBe("Assignment retrieved successfully");
