...
describe('submitAssessment', () => {
	const ticket_num1 = 490;
	const ticket_num2 = 4901;
	const ticket_num3 = 4902;
	const ticket_num4 = 4903;
	const ticket_num5 = 4904;
	...
	test('Student not exist', async () => {
        const selected_options = [
            {
                criteriaIdSelected: criteria_id1,
                optionIdSelected: rating_option_id1
            }
        ];
		const result = await submitAssessmentService(
			0,
	...
	test('assignment not exist', async () => {
		...
		expect(result.status).toBe('error');
		expect(result.message).toBe('Assignment id does not exist');
	});
});
