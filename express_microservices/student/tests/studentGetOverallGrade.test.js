...
describe('studentGetOverallGradeService', async () => {
	const instructor_id = 3341;
	const student_id1 = 33411;
	const student_id2 = 33412;
	const student_id3 = 33413;
	...
	beforeEach(async () => {
		try {
			await Instructor.create({
				...
				FIRST_NAME: 'FIRST_NAME3341',
				MIDDLE_NAME: 'MIDDLE_NAME3341',
				LAST_NAME: 'LAST_NAME3341',
				EMAIL: 'TEST3341@INSTRUCTOR.COM',
				MD5_HASHED_PASSWORD: 'MD5_PASSWORD',
			});
			await Student.create({
				STUDENT_ID: student_id1,
				FIRST_NAME: 'FIRST_NAME33411',
				MIDDLE_NAME: 'MIDDLE_NAME33411',
				LAST_NAME: 'LAST_NAME33411',
				EMAIL: 'STUDENT33411@STUDENT.COM',
				MD5_HASHED_PASSWORD: 'MD5_PASSWORD',
			});
			await Student.create({
				STUDENT_ID: student_id2,
				FIRST_NAME: 'FIRST_NAME33412',
				MIDDLE_NAME: 'MIDDLE_NAME33412',
				LAST_NAME: 'LAST_NAME33412',
				EMAIL: 'STUDENT33412@STUDENT.COM',
				MD5_HASHED_PASSWORD: 'MD5_PASSWORD',
			});
			await Student.create({
				STUDENT_ID: student_id3,
				FIRST_NAME: 'FIRST_NAME33413',
				MIDDLE_NAME: 'MIDDLE_NAME33413',
				LAST_NAME: 'LAST_NAME33413',
				EMAIL: 'STUDENT33413@STUDENT.COM',
				MD5_HASHED_PASSWORD: 'MD5_PASSWORD',
			});
			const courseCreated = await Course.create({
				INSTRUCTOR_ID: instructor_id,
				COURSE_NAME: 'TEST_COURSE_NAME3341',
				COURSE_CODE: 'COURSE_CODE3341',
				COURSE_SEMESTER: 'COURSE_SEMESTE3341',
				COURSE_YEAR: 'COURSE_YEAR3341',
				COURSE_TERM: 'COURSE_TERM3341',
				COURSE_VISIBILITY: 0,
			});
			...
			await EvaluationSection.create({
				...
				SECTION_NAME: 'SECTION_NAME3341-1',
				SECTION_WEIGHT: 40,
			});
			await EvaluationSection.create({
				FORM_ID: form_id1,
				QUESTION_ID: question_id2,
				SECTION_NAME: 'SECTION_NAME3341-2',
				SECTION_WEIGHT: 60,
			});
			await EvaluationSection.create({
				FORM_ID: form_id2,
				QUESTION_ID: question_id3,
				SECTION_NAME: 'SECTION_NAME3341-3',
				SECTION_WEIGHT: 100,
			});
	...
	test('Student does not exist', async () => {
		const result = await studentGetOverallGradeService(0, form_id1);
		expect(result.status).toBe('error');
		expect(result.message).toBe('Student id does not exist');
	});
	test('Form does not exist', async () => {
		const result = await studentGetOverallGradeService(student_id1, 0);
		expect(result.status).toBe('error');
		expect(result.message).toBe('Form id does not exist');
	});
	test('No one has evaluated this student yet', async () => {
		const result = await studentGetOverallGradeService(
			student_id1,
			form_id1
		);
