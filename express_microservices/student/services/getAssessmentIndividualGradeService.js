...
const viewIndividualAssessmentService = require('../services/viewIndividualAssessmentService');

async function getAssessmentIndividualGradeService(
	course_id,
	assignment_id,
	evaluator_id,
	evaluatee_id
	...
	try {
		...
		const assessment = await viewIndividualAssessmentService(
			course_id,
			assignment_id,
			evaluator_id,
			evaluatee_id
		);
		if (assessment.status === 'error' || assessment === null) {
			return {
				status: 'error',
		...
		if (assessment.rubric === undefined) {
			return {
				status: 'error',
		...
		return {
			status: 'error',
			message:
				'Error occured in getAssessmentIndividualGradeService' +
				error.message,
		};
	}
}

module.exports = getAssessmentIndividualGradeService;
