...
async function viewIndividualAssessmentService(
	course_id,
	assignment_id,
	evaluator_id,
	evaluatee_id
	...
	try {
		...
		const assignment_and_rubric = await getAssignmentService(
			evaluatee_id,
			course_id,
			assignment_id
		);
		...
		return {
			status: 'error',
			message:
				'Error occured while retrieving results in viewIndividualAssessmentService.js: ' +
				err.message,
		};
	}
}
...
module.exports = viewIndividualAssessmentService;
