...
async function getAssessmentOverallGradeService(
	course_id,
	assignment_id,
	evaluatee_id
) {
	...
	try {
		...
		for (let i = 0; i < evaluators.length; i++) {
			const grade = await getAssessmentIndividualGradeService(
				course_id,
				assignment_id,
		...
		return {
			status: 'error',
			message:
				'Error occured in getAssessmentOverallGradeService' +
				err.message,
		};
	}
}

module.exports = getAssessmentOverallGradeService;
